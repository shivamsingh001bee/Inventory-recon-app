...
export async function runMonthlyReconciliation(runBy: string, role: string, reconMonth: string): Promise<string> {
  ...
  await bq.query({
    query: `DELETE FROM ${table("recon_missing_from_master")} WHERE recon_month = @reconMonth`,
    params: { reconMonth }
  });
  await bq.query({
    query: `
  ...
  await bq.query({
    query: `DELETE FROM ${table("recon_missing_from_entries")} WHERE recon_month = @reconMonth`,
    params: { reconMonth }
  });
  await bq.query({
    query: `
  ...
  await bq.query({
    query: `DELETE FROM ${table("recon_out_of_stock_but_found")} WHERE recon_month = @reconMonth`,
    params: { reconMonth }
  });
  await bq.query({
    query: `
...
export async function fetchReport(
  ...
  const whereClause = ["recon_month = @reconMonth", ...priceFilters].join(" AND ");
